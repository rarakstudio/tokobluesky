...
switch($_GET[act]){
  ...
  default:
   ...
   $tampil=mysql_query("SELECT * from kategori ORDER BY id_kategori asc");
    ...
    while ($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
	  <td>$r[nama]</td>
	  <td><a href=?module=parentkategori&act=edit_parentkategori&id=$r[id_kategori]>Edit</a> | 
	                  ";?><a onclick="return confirm('Apakah anda yakin menghapus data ini?');" <?php echo"href=$aksi?module=parentkategori&act=hapus&id=$r[id_kategori]>Hapus</a>
		        </tr>";
    $no++;
    }
    echo "</tbody></table>";
  ...
  case "tambah_parentkategori":
    echo "<h2>TAMBAHKAN PARENT KATEGORI</h2>
          ...
          <table>
          <tr><td>Nama Parent Kategori</td><td>  : <input type=text name='nama' size=30></td></tr>
          <tr><td colspan=2>
  ...
  case "edit_parentkategori":
    ...
    echo "<h2>EDIT PARENT KATEGORI PRODUK</h2>
          ...
          <table>
		   <tr><td>Nama Parent Kategori</td><td>     : <input type=text name='nama' size=30 value='$r[nama]'></td></tr>
